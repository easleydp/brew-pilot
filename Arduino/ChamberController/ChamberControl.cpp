...
void forceFridge(ChamberData& cd, byte setting) {
  ...
  if (setting == ON) {
    ...
    if (!cd.fridgeOn) {
      logMsg(LOG_INFO, "CC", 'F', cd.chamberId, cd.fridgeLastToggleMins /* uint8_t */);
...
void heater(ChamberData& cd, uint8_t outputLevel) {
  ...
  if (outputLevel == 0 || outputLevel == 100)
    if (cd.heaterOutput != outputLevel)
      logMsg(LOG_INFO, "CC", 'H', cd.chamberId, outputLevel /* uint8_t */);
...
void controlChamber(ChamberData& cd) {
  ...
  if (tError == 0) {  // Zero error
    ...
    logMsg(LOG_DEBUG, "PID", 'X', chamberId, tErrorAdjustedForSawtooth, tExternalBoost);
  ...
  if (abs(integralContrib) > 50) {
    logMsg(LOG_DEBUG, "PID", 'W', chamberId, integralContrib /* float */);
  } else {
    cd.mParams.integral = latestIntegral;
  }

  logMsg(LOG_DEBUG, "PID", '~', chamberId, tError /* int16 */, cd.mParams.integral /* float */, cd.priorError /* float */);
  if (heatPidWise) {
    // Tuning insight - see what each PID factor is contributing:
    logMsg(LOG_DEBUG, "PID", 'C' /* PID output Components (3 floats) */,
    ...
    if (pidOutput < 0.0) {  // Oddly, heatPidWise is true but PID output is -ve. Can happen though, most commonly due to integral being -ve.
      logMsg(LOG_WARN, "PID", '!', chamberId, pidOutput /* float */);
      hSetting = 0;
      ...
      logMsg(params.hasHeater ? LOG_WARN : LOG_DEBUG, "PID", '+', chamberId, pidOutput /* float */);
      hSetting = 100;
    } else {  // pidOutput is within normal range
      logMsg(LOG_DEBUG, "PID", '-', chamberId, pidOutput /* float */);
  ...
  if (hSetting > 0) {
    ...
    if (cd.fridgeOn || cd.fridgeLastToggleMins < ANTI_SEESAW_MARGIN_MINS) {
      ...
      logMsg(LOG_DEBUG, "CC", 'C', chamberId, cd.fridgeLastToggleMins /* uint8_t */, hSetting /* byte */);
      hSetting = 0;
    }
  }
  if (fSetting == ON) {
    ...
    if (cd.heaterOutput > 0 || cd.heaterLastToggleMins < ANTI_SEESAW_MARGIN_MINS) {
      ...
      logMsg(LOG_DEBUG, "CC", 'D', chamberId, cd.heaterLastToggleMins /* uint8_t */);
      fSetting = OFF;
    }
  }
  ...
  if (cd.priorError - tError != 0) {
    ...
    logMsg(LOG_DEBUG, "PID", 'd', chamberId, cd.tBeerLastDelta /* int8 */);
...
void controlChambers() {
  if (TIME_UP(prevMillisChamberControl, uptimeMillis, CHAMBER_ITERATION_TIME_MILLIS)) {
    ...
    logMsg(LOG_DEBUG, "CC", 'j', 1, ((uint32_t)millis() - t) /* uint32_t */);
    readTExternal();
    readTProjectBox();
    for (byte i = 0; i < CHAMBER_COUNT; i++) {
      ChamberData& cd = chamberDataArray[i];
    ...
    logMsg(LOG_DEBUG, "CC", 'k', 1, ((uint32_t)millis() - t) /* uint32_t */);  // ~1100ms for 6 sensors
  }
  maintainHeaters();
}
